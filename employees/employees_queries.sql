...
to_date
FROM titles 
WHERE to_date = '9999-01-01' AND title IN ('Senior Engineer' , 'Engineer')) t1
INNER JOIN (
SELECT emp_no,
salary,
from_date,
...
SELECT COUNT(t3.dept_no) AS dept_no_freq, 
t3.dept_no, 
t3.dept_name FROM
(SELECT t1.dept_no, 
t2.dept_name
...
SELECT t4.dept_no, 
t4.dept_name, 
AVG(t4.salary) AS avg_sal_per_dept 
FROM (
SELECT t1.dept_no,
t1.emp_no, 
t1.from_date AS dept_from_date, 
t1.to_date AS dept_to_date,
t2.salary, 
t2.from_date AS salary_from_date, 
t2.to_date AS salary_to_date, 
t3.dept_name 
...
SELECT t1.title, 
COUNT(t1.title) AS title_freq, 
YEAR(t1.from_date) AS hire_date 
FROM titles t1

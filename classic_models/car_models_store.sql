...
t3.productName,
SUM(t3.quantityOrdered) AS total_quantity_ordered,
...
t1.quantityOrdered,
t1.priceEach,
t1.quantityOrdered * t1.priceEach AS product_revenue,
t2.productName
...
## -- profit per product
SELECT t4.productCode,productName,
t4.total_quantity_ordered,
t4.total_revenue_per_product,
t4.total_cost_per_product,
t4.total_revenue_per_product - t4.total_cost_per_product AS total_profit_per_product
FROM (
SELECT t3.productCode,t3.productName,
SUM(t3.quantityOrdered) AS total_quantity_ordered,
SUM(t3.product_revenue) AS total_revenue_per_product,
...
t2.productName, 
t2.buyPrice * t1.quantityOrdered AS product_cost
FROM orderdetails t1
LEFT JOIN products t2 ON t1.productCode = t2.productCode) t3
GROUP BY t3.productCode
...
SELECT t4.*, 
t4.total_revenue_per_product_line - t4.total_cost_per_product_line AS total_profit_per_product_line 
FROM (
SELECT t3.productLine,
SUM(t3.quantityOrdered) AS total_quantity_ordered_per_product_line, 
SUM(t3.product_revenue) AS total_revenue_per_product_line,

...
SELECT t6.*,
CASE WHEN t6.total_list_price_after_discount_sum > t6.avg_total_price THEN 'Above average' ELSE 'Below average' END AS total_price_category 
FROM (
SELECT t5.*,  
...
SELECT 
	order_id,
	SUM(list_price - (list_price*discount)) AS sum_final_price_by_order
FROM
	[sales].[order_items]
GROUP BY order_id )
SELECT 
...
SELECT 
	...
	FinalPriceByOrder = (
						SELECT 
							SUM(t2.list_price - (t2.list_price*t2.discount))
						FROM 
...
--- Calculate total list price (after discount) for each day of the week 
--- Order the data by the day of the week in an ascending order
--- Calculate the dynamic of the total list price (value from tuesday compared to monday and so on)

SELECT
	t2.*,
	((t2.TotalListPricePerDayOfWeek*1.0) / LAG(TotalListPricePerDayOfWeek,1) OVER(ORDER BY OrderDayOfWeekNumber ASC)) - 1 AS TotalListPriceDynamic
FROM 
(
SELECT
	t1.OrderDayOfWeek,
	CASE 
		WHEN t1.OrderDayOfWeek = 'Monday' THEN 1 
		WHEN t1.OrderDayOfWeek = 'Tuesday' THEN 2
		WHEN t1.OrderDayOfWeek = 'Wednesday' THEN 3 
		WHEN t1.OrderDayOfWeek = 'Thursday' THEN 4 
		WHEN t1.OrderDayOfWeek = 'Friday' THEN 5 
		WHEN t1.OrderDayOfWeek = 'Saturday' THEN 6 
	ELSE 7
	END AS 
		OrderDayOfWeekNumber,
	t1.TotalListPricePerDayOfWeek
FROM
(
SELECT 
	OrderDayOfWeek,
	SUM(TotalListPricePerOrder) AS TotalListPricePerDayOfWeek
FROM 
	#Temp1
GROUP BY
	OrderDayOfWeek
...
ORDER BY
	t2.OrderDayOfWeekNumber



--- Use declared variable to select specified columns where the list price is higher than the average list price
